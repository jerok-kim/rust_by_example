...
/// to create an iterator is to use the range notation `a..b`. This yields from `a`(inclusive)
/// to `b`(exclusive) in steps of one.
///
/// Let's write FizzBuss using `for` instead of `while.
...
/// As discussed in the section on the `Iterator` trait, by default the `for` loop will
/// apply the `into_iter` function to the collection.
...
/// - `into_iter` - This consumes the collection so that on each iteration the exact data
/// is provided. Once the collection has been consumed it is no longer available for reuse
...
/// In the above snippets note the type of `match` branch, that is the key difference
/// in the types of iteration. The difference in type then of course implies differing
