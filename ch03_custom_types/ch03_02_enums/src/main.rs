/// # Enums
///
/// The `enum` keyword allows the creation of a type which may be one of a few different variants. Any variant which is valid is a `struct` is also valid as an `enum`.
