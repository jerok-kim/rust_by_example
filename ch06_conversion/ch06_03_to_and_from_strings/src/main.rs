...
///
/// To convert any type to a `String` is as simeple as implementing the `ToString` trait for the type.
/// Rather than doing so directly, you should implement the `fmt::Display` trait
/// which automagically provides `ToString` and also allows printing the type as discussed
/// in the section on `print!`.
///
/// ## Parsing a String
///
/// One of the more common types to convert a string into is a number. The idiomatic approach
/// to this is to use the `parse` function and either to arrange for type inference or
...
/// implemented for that type. This is implemented numerous types within the standard library.
